...
namespace Simple.EventStore.Internal.Helpers
{
	...
	internal class ObjectMethodExecutor
    {
        ...
        private ObjectMethodExecutor(MethodInfo methodInfo, TypeInfo targetTypeInfo, object?[]? parameterDefaultValues)
        {
            ...
            this.MethodParameters = methodInfo.GetParameters();
            this.TargetTypeInfo = targetTypeInfo;
            this.MethodReturnType = methodInfo.ReturnType;

            var isAwaitable = CoercedAwaitableInfo.IsTypeAwaitable(this.MethodReturnType, out var coercedAwaitableInfo);

            this.IsMethodAsync = isAwaitable;
            this.AsyncResultType = isAwaitable ? coercedAwaitableInfo.AwaitableInfo.ResultType : null;
            ...
            this._executor = GetExecutor(methodInfo, targetTypeInfo);

            if (this.IsMethodAsync)
            {
                this._executorAsync = GetExecutorAsync(methodInfo, targetTypeInfo, coercedAwaitableInfo);
            }

            this._parameterDefaultValues = parameterDefaultValues;
        }
        ...
        {
            Debug.Assert(this._executor != null, "Sync execution is not supported.");
            return this._executor?.Invoke(target, parameters);
        }
        ...
        {
            var methodExecutorAsync = this._executorAsync;
            if (methodExecutorAsync != null)
            {
                return methodExecutorAsync(target, parameters);
            }

            throw new NotSupportedException("Async execution is not supported");
        }

        public object? GetDefaultValueForParameter(int index)
        {
            if (this._parameterDefaultValues == null)
            {
                throw new InvalidOperationException($"Cannot call {nameof(this.GetDefaultValueForParameter)}, because no parameter default values were supplied.");
            }
            ...
            return this._parameterDefaultValues[index];
        }
